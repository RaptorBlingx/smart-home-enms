...
The rapid growth of Internet of Things (IoT) technologies and increasing energy costs have created demand for intelligent home energy management systems. This project presents the design, implementation, and evaluation of a comprehensive smart home energy management system that integrates IoT devices, real-time data processing, and machine learning to enable monitoring, control, and prediction of residential energy consumption.

The system architecture comprises seven microservices: a device simulator generating realistic consumption data for five household appliances, an Eclipse Mosquitto MQTT broker facilitating lightweight message exchange, a Node-RED integration layer, a PostgreSQL database storing over 300,000 historical records, a FastAPI backend exposing 15 RESTful endpoints, a machine learning service implementing Linear Regression models with cyclical feature encoding, and a React-based web dashboard. The machine learning component achieves average R² score of 0.810, predicting consumption with 96.5% accuracy. Performance testing demonstrates end-to-end latency of 65ms, device control response time of 1.3 seconds, system throughput of 20+ requests per second, and 100% uptime over 24 hours with zero message loss across 43,200 processed messages.

The system successfully achieves all project objectives including real-time monitoring with sub-second latency, remote device control, ML predictions exceeding 70% accuracy target, comprehensive cost tracking, and identification of 12.3% potential energy savings. Key contributions include an open-source reference implementation, documented performance benchmarks, demonstration of cyclical hour encoding improving model performance by 8-12%, and a Docker-based deployment template. Future work encompasses real hardware integration, LSTM neural networks, anomaly detection, and evolution toward a multi-tenant SaaS platform.

**Keywords:** Internet of Things, Smart Home, Energy Management, Machine Learning, MQTT, Time-Series Prediction, Linear Regression, Docker, Microservices Architecture
...
1.2. Smart Home Technologies  
1.3. MQTT Protocol  
1.4. Machine Learning for Time-Series  
1.5. Energy Management Systems

**CHAPTER 2: LITERATURE REVIEW**  
2.1. Smart Home Energy Management Systems  
2.2. IoT-Based Monitoring Solutions  
2.3. Machine Learning for Energy Prediction  
...
The Internet of Things (IoT) represents a paradigm where everyday physical objects are embedded with sensors, software, and network connectivity, enabling them to collect and exchange data. This interconnected ecosystem transforms passive objects into intelligent devices capable of sensing their environment, communicating with other devices, and responding to user commands or automated triggers.

**Key IoT Characteristics:**
- **Connectivity:** Wi-Fi, Bluetooth, Zigbee, cellular networks
- **Sensing:** Temperature, humidity, motion, power consumption
- **Actuation:** Control physical devices remotely
- **Intelligence:** Edge computing and cloud processing
- **Interoperability:** Standardized protocols enable device integration

## 1.2. Smart Home Technologies

Smart home systems integrate IoT devices to provide automation, security, and energy management. Common components include smart thermostats, lighting systems, security cameras, and appliance monitors. These systems typically follow a layered architecture: devices/sensors (physical layer), gateways/hubs (network layer), cloud platforms (application layer), and user interfaces (presentation layer).

## 1.3. MQTT Protocol

MQTT (Message Queuing Telemetry Transport) is a lightweight publish-subscribe messaging protocol designed for resource-constrained devices. It operates on TCP/IP and uses a broker-based architecture where clients publish messages to topics and subscribers receive messages from topics they've subscribed to. MQTT's small code footprint, minimal bandwidth requirements, and support for Quality of Service (QoS) levels make it ideal for IoT applications.

**MQTT QoS Levels:**
- **QoS 0:** At most once delivery (fire and forget)
- **QoS 1:** At least once delivery (acknowledged delivery)
- **QoS 2:** Exactly once delivery (assured delivery with overhead)

## 1.4. Machine Learning for Time-Series Prediction

Time-series forecasting involves predicting future values based on historical observations. For energy consumption prediction, regression algorithms analyze patterns in historical data to forecast future usage. Linear Regression, despite its simplicity, performs well when proper feature engineering is applied. Advanced techniques include ARIMA (AutoRegressive Integrated Moving Average), LSTM neural networks, and Prophet.
...
Energy Management Systems (EMS) monitor, control, and optimize energy consumption in buildings. Modern EMS leverage IoT sensors for real-time data collection, cloud platforms for data storage and processing, and machine learning for predictive analytics. Key functions include consumption monitoring, load forecasting, demand response, and automated control strategies.
...
IoT-enabled energy monitoring systems have evolved from simple data loggers to comprehensive platforms. Key developments include:

- **Non-intrusive load monitoring (NILM):** Disaggregates total consumption into device-level usage without individual sensors
- **Edge computing:** Processes data locally to reduce latency and bandwidth
- **Standardization efforts:** Matter protocol aims to unify smart home ecosystems

## 2.3. Machine Learning for Energy Prediction

ML applications in energy forecasting span multiple algorithms:

- **Linear models:** Simple, interpretable, effective for short-term forecasts
- **Tree-based methods:** Random Forest, XGBoost handle non-linear relationships
- **Deep learning:** LSTM, GRU capture long-term dependencies
- **Hybrid approaches:** Combine multiple models for improved accuracy

## 2.4. Research Gap Analysis

Despite advances, current literature reveals gaps:

1. **Limited open-source implementations:** Most research presents algorithms without complete system code
2. **Simulated vs. real data:** Many studies use synthetic data, questioning generalizability
3. **Scalability validation:** Few papers test beyond 10-20 devices
4. **User-centric design:** Technical solutions often lack usability studies
5. **Cost-benefit analysis:** Economic viability rarely quantified

**This project addresses these gaps** by providing:
- Complete open-source implementation with Docker deployment
- Comprehensive testing methodology including performance benchmarks
- Detailed documentation enabling reproducibility
- Cost tracking and savings quantification
...
Despite the availability of smart home technologies and IoT devices, several critical gaps exist in current residential energy management solutions:

**1. Limited Real-Time Visibility:** Most homeowners receive energy consumption information only through monthly utility bills, which provide aggregated data that offers little insight into specific device consumption or usage patterns. This lack of granular, real-time information makes it difficult for users to identify energy-intensive devices or understand the impact of their daily activities on overall consumption.

**2. Absence of Predictive Capabilities:** Existing consumer-grade energy monitoring solutions typically focus on historical data visualization without incorporating predictive analytics. Users cannot anticipate future energy consumption or peak load periods, limiting their ability to make proactive decisions about energy usage.

**3. Device-Level Control Limitations:** While some smart home platforms offer device control capabilities, these are often vendor-specific and lack integration with comprehensive energy monitoring. Users cannot easily correlate energy consumption data with device states or implement automated control strategies based on energy optimization goals.

**4. Cost Tracking Deficiencies:** Many energy monitoring systems display consumption in kilowatt-hours (kWh) without translating this technical metric into monetary costs. This abstraction makes it challenging for users to understand the financial implications of their energy usage and evaluate the cost-effectiveness of efficiency measures.

**5. Complexity and Accessibility:** Commercial energy management systems are often complex, expensive, and require professional installation. This high barrier to entry limits adoption among average homeowners who could benefit most from energy optimization.

**6. Lack of Integration:** Different aspects of smart home energy management—monitoring, control, prediction, and visualization—are typically provided by separate systems that do not communicate effectively, resulting in fragmented user experiences and missed optimization opportunities.

These limitations highlight the need for an integrated, accessible, and intelligent energy management system that combines real-time monitoring, predictive analytics, bidirectional device control, and intuitive visualization in a cohesive platform.

## Project Objectives

The primary aim of this graduation project is to design, implement, and evaluate a comprehensive Smart Home Energy Management System that addresses the identified gaps in current solutions. The specific objectives are:

**1. Real-Time Energy Monitoring:** Develop a system capable of collecting and processing energy consumption data from multiple devices at high temporal resolution (10-second intervals), providing users with up-to-date information about their energy usage patterns.

**2. Predictive Analytics Implementation:** Integrate machine learning algorithms to analyze historical consumption data and generate predictions for future energy usage at the device level, enabling proactive energy management decisions.

**3. Bidirectional Device Control:** Implement a control mechanism that allows users to remotely monitor device states and issue commands to turn devices on or off, facilitating manual energy optimization and supporting automated control scenarios.

**4. Cost Calculation and Tracking:** Incorporate real-time cost calculation functionality that translates energy consumption into monetary values based on configurable electricity rates, making financial implications immediately visible to users.

**5. Multi-Channel Data Visualization:** Create intuitive user interfaces that present energy data through multiple channels—a responsive web dashboard for detailed analysis and Grafana dashboards for real-time monitoring—catering to different use cases and user preferences.

**6. Scalable Microservices Architecture:** Design and implement a modular, containerized architecture that separates concerns, enables independent scaling of components, and facilitates future extensions and modifications.

**7. IoT Protocol Implementation:** Utilize industry-standard IoT communication protocols, specifically MQTT (Message Queuing Telemetry Transport), to enable efficient, reliable, and lightweight data transmission between devices and the central system.

**8. Performance Evaluation:** Conduct comprehensive testing to evaluate system performance in terms of prediction accuracy, response times, data throughput, and overall reliability under realistic usage scenarios.
...
**System Components:**
- Energy data simulation service representing five common household appliances (Washing Machine, Refrigerator, Air Conditioner, Dishwasher, Water Heater)
- MQTT message broker for IoT communication
- Node-RED flow-based integration layer
- PostgreSQL relational database for data persistence
- FastAPI-based RESTful backend API with 22 endpoints
- React 18-based single-page application (SPA) frontend
- Grafana visualization platform for real-time dashboards

**Functional Capabilities:**
- Real-time energy consumption monitoring with 10-second update intervals
- Historical data storage and retrieval spanning multiple days
- Device state monitoring and control (ON/OFF operations)
- Machine learning-based energy consumption prediction using Linear Regression
- Cost calculation based on configurable electricity rates ($0.12/kWh default)
- Data aggregation and analytics (daily, weekly, monthly statistics)
- Device efficiency ranking and comparison
- Peak consumption period identification

**Technical Implementation:**
- Containerized deployment using Docker and Docker Compose
- Microservices architecture with seven distinct services
- RESTful API design following industry best practices
- Responsive frontend design supporting multiple screen sizes
- Feature engineering for ML models (temporal features, cyclical encoding)
- Per-device model training and prediction

### Limitations

The following limitations are acknowledged:

**1. Simulated Data Environment:** The system utilizes simulated device data rather than actual hardware sensors. While simulation patterns are based on realistic consumption profiles with appropriate variations, they may not capture all nuances of real-world device behavior.

**2. Limited Device Diversity:** The implementation focuses on five specific appliance types. Expansion to additional device categories would require additional simulation logic and potentially different modeling approaches.

**3. Machine Learning Model Simplicity:** The project employs Linear Regression for energy prediction, which, while effective for demonstration and baseline performance, represents a relatively simple approach. More sophisticated models (e.g., LSTM networks, Prophet, ensemble methods) could potentially achieve higher accuracy.

**4. Security Implementation:** The current implementation does not include comprehensive security features such as authentication, authorization, encrypted communications, or secure device pairing. These would be essential for production deployment.

**5. Single-User/Single-Home Focus:** The system is designed for a single household environment. Multi-user support, multi-home management, and user roles are not implemented.

**6. Network Requirements:** The system assumes reliable network connectivity between components. Network failure handling and offline operation capabilities are limited.

**7. Scalability Testing:** While the architecture is designed for scalability, extensive load testing with hundreds of devices has not been conducted within the project scope.

**8. Historical Data Depth:** Machine learning models require seven days of historical data for training. Prediction capabilities are not available until this threshold is met.

## Report Organization

This report is structured to provide a comprehensive account of the project's development, implementation, and evaluation:

**Chapter 1: IoT and Smart Home Energy Management** provides theoretical background on Internet of Things technologies, smart home applications, communication protocols (particularly MQTT), and energy monitoring systems. This chapter establishes the technical foundation for understanding the project's context.

**Chapter 2: Literature Review** surveys existing research and commercial solutions in smart home energy management, MQTT implementations, machine learning for energy prediction, and microservices architectures. The chapter concludes with a gap analysis that positions this project within the current landscape.

**Chapter 3: System Design and Architecture** presents the overall system design, including high-level architecture, component descriptions, data flow diagrams, database schema, and design decisions. This chapter explains the "what" and "why" of the system's structure.

**Chapter 4: Materials and Methodology** details the development environment, technology stack, implementation methodology, data collection strategies, machine learning approach, and development workflow. This chapter focuses on the "how" of system development.

**Chapter 5: Implementation** provides detailed descriptions of each component's implementation, including code snippets, configuration files, and technical explanations. This chapter demonstrates the practical realization of the design presented in Chapter 3.

**Chapter 6: Testing and Results** presents the testing methodology, functional and performance testing outcomes, machine learning model evaluation metrics, and analysis of results. This chapter validates the system's capabilities against the stated objectives.

**Chapter 7: Discussion** interprets the results, analyzes the system's strengths and limitations, compares the implementation with existing solutions, and reflects on challenges encountered and lessons learned.

**Conclusion and Recommendations** synthesizes the project's contributions, evaluates achievement of objectives, and proposes directions for future work and system enhancements.

**References** provides a comprehensive list of academic publications, technical documentation, and other sources cited throughout the report.

**Appendices** include supplementary materials such as complete API documentation, database schemas, machine learning code, configuration files, user guides, and detailed test results.

---

*[End of Introduction - Ready for Chapter 1]*
...
This chapter presents the comprehensive design and architecture of the Smart Home Energy Management System. It describes the overall system structure, design principles, individual components, data flow mechanisms, database schema, and key architectural decisions that shape the implementation.

## 3.1. Overall System Overview

The Smart Home Energy Management System is architected as a distributed microservices-based platform, leveraging containerization technology to achieve modularity, scalability, and maintainability. The system comprises seven independent services that communicate through well-defined interfaces, enabling loose coupling and independent evolution of components.

### 3.1.1. High-Level Architecture

The architecture follows a layered approach organized into four primary tiers:

**1. Data Generation Layer:** Simulates IoT devices that generate energy consumption data at 10-second intervals, representing five common household appliances.

**2. Communication Layer:** Implements MQTT (Message Queuing Telemetry Transport) protocol for lightweight, publish-subscribe messaging between devices and the central system.

**3. Processing and Storage Layer:** Consists of data transformation services (Node-RED), persistent storage (PostgreSQL database), and business logic implementation (FastAPI backend).

**4. Presentation Layer:** Provides dual visualization channels—a React-based web application for detailed interaction and Grafana dashboards for real-time monitoring.
